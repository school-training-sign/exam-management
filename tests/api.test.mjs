...
test("데모 로그인은 허용된 접속 이름과 숫자 4~6자리 PIN만 받는다", async () => {
  ...
  await assert.rejects(
    repository.request("user_login", { login_name: "테스트교사", pin: "123" }),
    (error) => error.code === "INVALID_LOGIN",
  );
  await assert.rejects(
    repository.request("user_login", { login_name: "테스트교사", pin: "1234567" }),
    (error) => error.code === "INVALID_LOGIN",
  );
...
test("관리자는 접속 사용자를 추가·수정·비활성화하고 PIN 재설정으로 이전 세션을 만료시킨다", async () => {
  ...
  setUserSession("demo-user-session:system-admin");
  setAdminSession("demo-admin-session:system-admin");
  await repository.request("reset_access_user_pin", { id: user.id, pin: "11111" });
  setUserSession(firstLogin.user_session);
  await assert.rejects(
  ...
  await assert.rejects(
    repository.request("user_login", { login_name: "새 교사", pin: "11111" }),
    (error) => error.code === "INVALID_LOGIN",
  );
});

test("사용자 추가와 PIN 재설정은 숫자 4~6자리 규칙을 함께 적용한다", async () => {
  const repository = new DemoRepository();
  sessionStorage.clear();
  setUserSession("demo-user-session:system-admin");
  setAdminSession("demo-admin-session:system-admin");
  ...
  await assert.rejects(
    repository.request("save_access_user", { login_name: "짧은 PIN", pin: "123" }),
    (error) => error.code === "INVALID_PIN" && error.message === "PIN은 숫자 4~6자리로 입력하세요.",
  );
  await assert.rejects(
    repository.request("save_access_user", { login_name: "긴 PIN", pin: "1234567" }),
  ...
  const user = created.access_users.find((item) => item.login_name === "네 자리 교사");
  assert.ok(user);

  await repository.request("reset_access_user_pin", { id: user.id, pin: "13579" });
  await assert.rejects(
    repository.request("reset_access_user_pin", { id: user.id, pin: "12가4" }),
    (error) => error.code === "INVALID_PIN",
  );
  ...
  assert.equal(login.current_user.id, user.id);
});
